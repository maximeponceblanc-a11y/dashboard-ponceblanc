...
def load_synthese_data():
    url_github = "https://raw.githubusercontent.com/maxence2608/streamlit/main/Query_tableau_devis.xlsx"
...
# ========================================================
# ENTÊTE : FILTRES TEMPORELS GLOBAUX
# ========================================================
...
with st.container(border=True):
    ...
    with col_top_annee:
        ...
        annees_selectionnees = st.multiselect("Années d'analyse :", options=annees_possibles, default=annees_possibles)
        
    with col_top_mois:
        start_month, end_month = st.select_slider("Intervalle des mois inclus :", options=liste_mois_noms, value=("Janvier", "Décembre"))
...
def extraire_kpis_annee(dataframe, annee_cible):
    ...
    return {"ca_devis": ca_devis, "ca_signe": ca_signe, "tx_ca": tx_succes_ca, "vol_devis": vol_devis, "vol_signe": vol_signe, "tx_vol": tx_succes_vol}

# ========================================================
# RENDER : PERFORMANCE COMMERCIALE
# ========================================================
            ...
            with tabs_val[idx]:
                ...
                if kpis_current:
                    def générer_html_delta(val_curr, kpis_ref, cle, label_annee, unite="€", is_pct=False):
                        if not kpis_ref: return f"<span style='color: #94a3b8;'>vs {label_annee} : -</span>"
                        val_ref = kpis_ref[cle]
                        diff = val_curr - val_ref
                        color = "#22c55e" if diff > 0.001 else ("#ef4444" if diff < -0.001 else "#64748b")
                        arrow = "↗" if diff > 0.001 else ("↘" if diff < -0.001 else "→")
                        sign = "+" if diff > 0.001 else ""
                        val_txt = f"{sign}{diff:.2f} pts".replace('.', ',') if is_pct else f"{sign}{diff:,.0f} {unite}".replace(',', ' ')
    ...
    if not df_filtered_base.empty:
        ...
        if not df_marge.empty:
            ...
            with col_table:
                ...
                colonnes_mapping = {'Nom Client': 'Nom Client', 'DEVIS N°': 'DEVIS N°', 'Statut Devis': 'Statut Devis', 'Nb exemplaires': 'Quantité', 'Prix Unitaire': 'Prix unité', 'Taux Marge': 'Coef marge', 'Prix total': 'CA'}
                ...
                if 'CA' in df_table_display.columns: df_table_display['CA'] = df_table_display['CA'].apply(lambda x: f"{x:,.0f} €".replace(',', ' '))
                if 'Prix unité' in df_table_display.columns: df_table_display['Prix unité'] = df_table_display['Prix unité'].apply(lambda x: f"{x:,.2f}".replace('.', ','))
                if 'Coef marge' in df_table_display.columns: df_table_display['Coef marge'] = df_table_display['Coef marge'].apply(lambda x: f"{x:,.2f}".replace('.', ','))
