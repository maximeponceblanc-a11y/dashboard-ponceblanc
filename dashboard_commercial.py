...
import datetime
import io
import requests  # Ajouté pour garantir le téléchargement du flux binaire Excel
...
def load_synthese_data():
    ...
    try:
        # Téléchargement sécurisé du fichier Excel depuis l'URL Raw de GitHub
        response = requests.get(url_github)
        response.raise_for_status()  # Lève une erreur si l'URL est inaccessible
        ...
        st.info("Vérifiez que le fichier existe bien sur votre branche principale et que le dépôt est accessible.")
        st.stop()
