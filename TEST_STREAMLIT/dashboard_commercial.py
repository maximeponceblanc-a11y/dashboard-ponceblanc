...
# --- CHARGEMENT DE LA FEUILLE DE SYNTHÈSE DEPUIS SHAREPOINT ---
@st.cache_data
def load_synthese_data():
    chemin_fichier = "Query_tableau_devis.xlsx"
    try:
        df = pd.read_excel(chemin_fichier, sheet_name="PONCEBLANC")
    ...
    # Gestion des dates et création forcée des colonnes
    if 'Dates' in df.columns:
        df['Dates_Propres'] = pd.to_datetime(df['Dates'], errors='coerce')
        df = df.dropna(subset=['Dates_Propres'])
        df['Mois_Num'] = df['Dates_Propres'].dt.month
        # --- MODIFICATION : Création explicite de la colonne ---
        df['Mois_Nom'] = df['Dates_Propres'].dt.strftime('%B') 
        df['Année_Calculée'] = df['Dates_Propres'].dt.year.fillna(0).astype(int)
    else:
        # Valeurs par défaut si pas de dates
        df['Mois_Num'] = 1
        df['Mois_Nom'] = "Janvier"
        df['Année_Calculée'] = 2026

    # Sécurisation 'Année Devis'
    if 'Année Devis' in df.columns:
        df['Année Devis'] = pd.to_numeric(df['Année Devis'], errors='coerce').fillna(df['Année_Calculée']).astype(int)
    else:
        df['Année Devis'] = df['Année_Calculée']
...
try:
    ...
    st.sidebar.success(f"Base SharePoint synchronisée : {len(df)} lignes.")
except Exception as e:
    st.stop()
...
# 1. Années (basé sur la colonne sécurisée dans load_synthese_data)
if 'Année Devis' in df.columns:
    annees_disponibles = sorted([int(a) for a in df['Année Devis'].unique() if a > 2000], reverse=True)
else:
    annees_disponibles = [2026, 2025]

# 2. Sélecteur de période (Mois)
st.sidebar.subheader("Période d'analyse (Mois)")
liste_mois_noms = [
...
start_month, end_month = st.sidebar.select_slider(
    "Sélectionnez les mois inclus :",
    options=liste_mois_noms,
    value=("Janvier", "Avril")
)

# Conversion en numéros pour filtrer
mois_debut_num = liste_mois_noms.index(start_month) + 1
mois_fin_num = liste_mois_noms.index(end_month) + 1
mask_mois = (df['Mois_Num'] >= mois_debut_num) & (df['Mois_Num'] <= mois_fin_num)

# 3. Filtres métier
st.sidebar.subheader("Filtres Métier")
...
# Création du DataFrame filtré final
df_filtered_base = df[mask_metier & mask_mois].copy()
...
# --- FONCTION DE CALCUL DES KPIS POUR UNE ANNÉE DONNÉE ---
...
if page == "📈 Performance Commerciale":
    ...
    st.info(f"📅 **Filtre Période active** : Analyse ciblée de **{start_month}** à **{end_month}**.")
    
    if len(annees_disponibles) > 0:
        tabs_val = st.tabs([f"Année {annee}" for annee in annees_disponibles])
        
        for idx, annee_selectionnee in enumerate(annees_disponibles):
            with tabs_val[idx]:
                
                # -------------------------------------------------------------
                # SECTION 1 : KPIS DE L'ONGLET REORDONNÉS
                # -------------------------------------------------------------
                st.subheader(f"📌 Indicateurs Clés — {annee_selectionnee} ({start_month} à {end_month})")
                ...
                if kpis_current:
                    # Sécurisation des chaînes pour éviter le plantage ValueError
                    def générer_html_delta(val_curr, kpis_ref, cle, label_annee, unite="€", is_pct=False):
                        if not kpis_ref:
                            return f"<span style='color: #94a3b8;'>vs {label_annee} : Pas de données</span>"
                    ...
                    h_ca_devis_prev = générer_html_delta(kpis_current["ca_devis"], kpis_prev, "ca_devis", annee_prev, "€")
                    h_ca_devis_next = générer_html_delta(kpis_current["ca_devis"], kpis_next, "ca_devis", annee_next, "€")
                    
                    h_ca_signe_prev = générer_html_delta(kpis_current["ca_signe"], kpis_prev, "ca_signe", annee_prev, "€")
                    h_ca_signe_next = générer_html_delta(kpis_current["ca_signe"], kpis_next, "ca_signe", annee_next, "€")
                    
                    h_tx_ca_prev = générer_html_delta(kpis_current["tx_ca"], kpis_prev, "tx_ca", annee_prev, is_pct=True)
                    h_tx_ca_next = générer_html_delta(kpis_current["tx_ca"], kpis_next, "tx_ca", annee_next, is_pct=True)
                    
                    h_vol_devis_prev = générer_html_delta(kpis_current["vol_devis"], kpis_prev, "vol_devis", annee_prev, "devis")
                    h_vol_devis_next = générer_html_delta(kpis_current["vol_devis"], kpis_next, "vol_devis", annee_next, "devis")
                    
                    h_vol_signe_prev = générer_html_delta(kpis_current["vol_signe"], kpis_prev, "vol_signe", annee_prev, "signés")
                    h_vol_signe_next = générer_html_delta(kpis_current["vol_signe"], kpis_next, "vol_signe", annee_next, "signés")
                    
                    # CORRECTION DU ANNEE_PREV
                    h_tx_vol_prev = générer_html_delta(kpis_current["tx_vol"], kpis_prev, "tx_vol", annee_prev, is_pct=True)
                    h_tx_vol_next = générer_html_delta(kpis_current["tx_vol"], kpis_next, "tx_vol", annee_next, is_pct=True)

                    # --- Ligne 1 : CA Devisé | CA Signé | % Succès (€) ---
                    c1, c2, c3 = st.columns(3)

                    with c1:
                        st.metric(label=f"CA Signé {annee_selectionnee}", 
                                  value=f"{kpis_current['ca_signe']:,.0f} €".replace(',', ' '))
                        st.markdown(f"{h_ca_signe_prev} &nbsp;|&nbsp; {h_ca_signe_next}", unsafe_allow_html=True)
                    with c2:
                        st.metric(label=f"CA Devisé {annee_selectionnee}", 
                                  value=f"{kpis_current['ca_devis']:,.0f} €".replace(',', ' '))
                        st.markdown(f"{h_ca_devis_prev} &nbsp;|&nbsp; {h_ca_devis_next}", unsafe_allow_html=True)
                    with c3:
                        st.metric(label="% Succès (€)", 
                                  value=f"{kpis_current['tx_ca']:.2f} %".replace('.', ','))
                        st.markdown(f"{h_tx_ca_prev} &nbsp;|&nbsp; {h_tx_ca_next}", unsafe_allow_html=True)
                    
                    st.write("") 
                    
                    # --- Ligne 2 : Devis Émis | Devis Signés | % Succès (Volume) ---
                    c4, c5, c6 = st.columns(3)

                    with c4:
                        st.metric(label="Nombre de devis signés", value=f"{kpis_current['vol_signe']}")
                        st.markdown(f"{h_vol_signe_prev} &nbsp;|&nbsp; {h_vol_signe_next}", unsafe_allow_html=True)

                    with c5:
                        st.metric(label="Nombre de devis émis", value=f"{kpis_current['vol_devis']}")
                        st.markdown(f"{h_vol_devis_prev} &nbsp;|&nbsp; {h_vol_devis_next}", unsafe_allow_html=True)
                    with c6:
                        st.metric(label="% Succès (Volume)", value=f"{kpis_current['tx_vol']:.2f} %".replace('.', ','))
                        st.markdown(f"{h_tx_vol_prev} &nbsp;|&nbsp; {h_tx_vol_next}", unsafe_allow_html=True)
                
                st.divider()
                
                # -------------------------------------------------------------
                # SECTION 2 : GRAPHIQUES ET TABLEAUX ASSOCIES
                # -------------------------------------------------------------
                ...
                if not df_annee.empty:
                    ...
                    with col_gauche:
                        ...
                        base_ca = df_annee.groupby('Mois_Nom')['Prix total'].sum().reset_index(name='Chiffre d\'affaire')
                        df_annee_signe = df_annee[df_annee["Signé?"].astype(str).str.upper().str.strip() == "O"]
                        signe_ca = df_annee_signe.groupby('Mois_Nom')['Prix total'].sum().reset_index(name='CA signé')
                        ...
                        df_graph = df_graph.sort_values('Mois_Nom')
                        ...
                        fig.update_layout(barmode='group', xaxis=dict(tickangle=-45), yaxis=dict(title="Montant (€)", showgrid=True), yaxis2=dict(title="Taux succès (€)", tickformat=".2%", range=[0, 1], showgrid=False), legend=dict(orientation="h", y=1.1, x=0), hovermode="x unified", height=380)
                        ...
                        base_vol = df_annee.groupby('Mois_Nom')[col_id_devis].nunique().reset_index(name='Nb devis')
                        signe_vol = df_annee_signe.groupby('Mois_Nom')[col_id_devis].nunique().reset_index(name='Nb devis signés')
                        ...
                        df_graph_vol = df_graph_vol.sort_values('Mois_Nom')
                        ...
                        fig_vol.update_layout(barmode='group', xaxis=dict(tickangle=-45), yaxis=dict(title="Nombre de devis", showgrid=True), yaxis2=dict(title="Taux succès (vol)", tickformat=".2%", range=[0, 1], showgrid=False), legend=dict(orientation="h", y=1.1, x=0), hovermode="x unified", height=380)
                    ...
                    with col_droite:
                        ...
                        fig_pie = go.Figure(data=[go.Pie(
                            labels=df_pie['Catégorie Client ABC'],
                            values=df_pie['Total CA'],
                            hole=.4,
                            marker=dict(colors=[colors_map.get(x, "#94a3b8") for x in df_pie['Catégorie Client ABC']]),
                            textinfo='percent',
                            hovertemplate="<b>%{label}</b><br>CA total : %{value:,.0f} €<br>Part : %{percent}<extra></extra>"
                        )])
                        fig_pie.update_layout(legend=dict(orientation="h", y=-0.1, x=0), margin=dict(t=10, b=10, l=10, r=10), height=320)
                        st.plotly_chart(fig_pie, use_container_width=True)
                        
                        st.markdown(f"**📋 Liste des dossiers de devis réalisés ({annee_selectionnee})**")
                    ...
                    st.info(f"Aucune ligne de détail à afficher pour l'année {annee_selectionnee}.")
    else:
        st.info("💡 Aucune année valide détectée dans vos données.")


# --- VUE 2 : MARGE & TARIFICATION ---
elif page == "💰 Analyse Marge & Prix":
    st.title("Analyse de la Marge & Règles de Prix")
    
    # Récupération préalable des années disponibles pour construire le sélecteur
    if 'Année Devis' in df_filtered_base.columns:
        annees_marge = sorted([int(a) for a in df_filtered_base['Année Devis'].unique() if a > 2000], reverse=True)
    else:
        annees_marge = [2026]
        
    # On initialise temporairement les données pour calculer les min/max des sliders
    df_marge_init = df_filtered_base[df_filtered_base['Année Devis'].isin(annees_marge)].copy()
    
    if not df_marge_init.empty:
        # Sécurisation des colonnes numériques
        df_marge_init['Prix Unitaire'] = pd.to_numeric(df_marge_init['Prix Unitaire'], errors='coerce').fillna(0)
        df_marge_init['Taux Marge'] = pd.to_numeric(df_marge_init['Taux Marge'], errors='coerce').fillna(0)
        
        # Récupération des bornes min/max dynamiques sur l'ensemble
        min_prix = float(df_marge_init['Prix Unitaire'].min())
        max_prix = float(df_marge_init['Prix Unitaire'].max())
        min_marge = float(df_marge_init['Taux Marge'].min())
        max_marge_val = float(df_marge_init['Taux Marge'].max())
        
        # Sécurisation si min == max pour éviter une erreur Streamlit
        if min_prix == max_prix: max_prix = min_prix + 1.0
        if min_marge == max_marge_val: max_marge_val = min_marge + 1.0

        st.write("---")
        st.markdown("**🎛️ Filtres de la vue**")
        
        # --- ALIGNEMENT SUR UNE SEULE LIGNE (3 COLONNES) ---
        col_annee, col_prix, col_marge = st.columns(3)
        
        # Colonne 1 : Choix des années
        with col_annee:
            annees_selectionnees = st.multiselect(
                "Années d'analyse", 
                options=annees_marge,
                default=[annees_marge[0]] if annees_marge else []
            )
            
        # Colonne 2 : Filtre Prix unité
        with col_prix:
        ...
        # Colonne 3 : Filtre Coef Marge
        with col_marge:
            marge_range = st.slider(
                "Coef marge",
                min_value=min_marge,
                max_value=max_marge_val,
                value=(min_marge, max_marge_val),
                step=0.01
            )
            
        # Application simultanée de tous les filtres (Années + Sliders)
        df_marge = df_filtered_base[
            (df_filtered_base['Année Devis'].isin(annees_selectionnees)) &
            (pd.to_numeric(df_filtered_base['Prix Unitaire'], errors='coerce').fillna(0) >= prix_range[0]) & 
            (pd.to_numeric(df_filtered_base['Prix Unitaire'], errors='coerce').fillna(0) <= prix_range[1]) &
            (pd.to_numeric(df_filtered_base['Taux Marge'], errors='coerce').fillna(0) >= marge_range[0]) & 
            (pd.to_numeric(df_filtered_base['Taux Marge'], errors='coerce').fillna(0) <= marge_range[1])
        ].copy()
        
        # Réassignation propre des types après filtrage pour la suite des calculs et graphiques
        if not df_marge.empty:
            df_marge['Prix Unitaire'] = pd.to_numeric(df_marge['Prix Unitaire'], errors='coerce').fillna(0)
            df_marge['Taux Marge'] = pd.to_numeric(df_marge['Taux Marge'], errors='coerce').fillna(0)
            ...
            # Création d'une colonne statut propre
            def get_statut(x):
            ...
            with col_table:
                ...
                colonnes_mapping = {
                    ...
                    'DEVIS N°': 'DEVIS N°',
                    'Statut Devis': 'Statut Devis',
                    'Nb exemplaires': 'Quantité',
                    'Prix Unitaire': 'Prix unité',
                    'Taux Marge': 'Coef marge',
                    'Prix total': 'CA'
                }
            ...
            with col_pie:
                ...
                fig_pie = go.Figure(data=[go.Pie(
                    labels=df_pie['Statut Devis'],
                    values=df_pie['Prix total'],
                    hole=.5, 
            ...
            if not df_signe.empty and 'Taux Marge' in df_signe.columns:
                ...
                agg_signe = df_signe.groupby(group_cols).agg(
                    CA_signe=('Prix total', 'sum'),
                    Somme_Marge_Pond=('Marge_Ponderee_Temp', 'sum')
                ).reset_index()
                
                agg_signe['Coef marge'] = agg_signe.apply(
                    lambda row: row['Somme_Marge_Pond'] / row['CA_signe'] if row['CA_signe'] > 0 else 0, 
                    axis=1
                )
            else:
                empty_cols = group_cols + ['CA_signe', 'Coef marge']
                agg_signe = pd.DataFrame(columns=empty_cols)
                
            df_combo = pd.merge(ca_devise, agg_signe[group_cols + ['CA_signe', 'Coef marge']], 
                                on=group_cols, how='left').fillna(0)
            df_combo = df_combo.sort_values(sort_col)
            df_combo[x_axis_col] = df_combo[x_axis_col].astype(str)
            
            fig_combo = make_subplots(specs=[[{"secondary_y": True}]])
            
            fig_combo.add_trace(go.Bar(
                x=df_combo[x_axis_col], y=df_combo['CA devisé'], 
                name="CA devisé", marker_color='#cccccc'
            ), secondary_y=False)
            
            fig_combo.add_trace(go.Bar(
                x=df_combo[x_axis_col], y=df_combo['CA_signe'], 
                name="CA signé", marker_color='#4287f5'
            ), secondary_y=False)
            ...
            fig_combo.add_trace(go.Scatter(
                x=df_combo[x_axis_col], y=df_combo['Coef marge'], 
                name="Coef marge", mode='lines+markers', 
                line=dict(color='#b370d6', width=2),
                marker=dict(size=8)
            ), secondary_y=True)
            
            fig_combo.update_layout(
                barmode='group',
                xaxis=dict(type='category', tickangle=0 if len(annees_selectionnees) > 1 else -45),
                yaxis=dict(title="CA devisé | CA signé (€)", showgrid=True),
                yaxis2=dict(title="Coef marge", range=[0, echelle_marge_max], showgrid=False),
            ...
            # ==========================================
            # BAS : GRILLE DE ANALYSE CROISÉE (2x2)
            # ==========================================
            st.markdown("**🔬 Modèles de tarification & Corrélations (Taille des bulles = CA global)**")
            
            # Paramétrage de la palette graphique pour l'homogénéité avec le camembert
            color_map_bubble = {"Signé": "#f59e42", "Refusé": "#4287f5", "En attente": "#94a3b8"}
            
            # S'assurer qu'il n'y a pas de valeurs négatives ou nulles strictes dans la taille pour éviter des bugs Plotly
            ...
            with col_g1:
                ...
                if 'Nombre de références totales' in df_marge.columns:
                    fig_g1 = px.scatter(
                        df_marge, x="Nombre de références totales", y="Taux Marge",
                        size="Taille_Bulle_CA", color="Statut Devis",
                        color_discrete_map=color_map_bubble, size_max=30,
                        hover_name="Nom Client", hover_data=["DEVIS N°", "Prix total"]
                    )
            ...
            with col_g2:
                ...
                fig_g2 = px.scatter(
                    df_marge, x="Nb exemplaires", y="Taux Marge",
                    size="Taille_Bulle_CA", color="Statut Devis",
                    color_discrete_map=color_map_bubble, size_max=30,
                    hover_name="Nom Client", hover_data=["DEVIS N°", "Prix total"]
                )
            ...
            with col_g3:
                ...
                if 'Nombre de références totales' in df_marge.columns:
                    fig_g3 = px.scatter(
                        df_marge, x="Nombre de références totales", y="Prix Unitaire",
                        size="Taille_Bulle_CA", color="Statut Devis",
                        color_discrete_map=color_map_bubble, size_max=30,
                        hover_name="Nom Client", hover_data=["DEVIS N°", "Prix total"]
                    )
            ...
            with col_g4:
                ...
                fig_g4 = px.scatter(
                    df_marge, x="Nb exemplaires", y="Prix Unitaire",
                    size="Taille_Bulle_CA", color="Statut Devis",
                    color_discrete_map=color_map_bubble, size_max=30,
                    hover_name="Nom Client", hover_data=["DEVIS N°", "Prix total"]
                )
        ...
        st.info("Aucune donnée disponible pour la ou les années sélectionnées.")