...
# --- CHARGEMENT DE LA FEUILLE DE SYNTHÈSE ---
@st.cache_data
def load_synthese_data():
    ...
    # CORRECTION CRUCIALE : Gestion des dates au format Numérique Excel (Série) et format standard
    if 'Dates' in df.columns:
        # On tente d'abord de convertir les codes numériques d'Excel (ex: 46012)
        ...
        # Si la ligne était déjà du texte ou une date standard, on complète avec le parsing classique
        df['Dates_Propres'] = df['Dates_Propres'].fillna(pd.to_datetime(df['Dates'], errors='coerce'))
        # Nettoyage des lignes n'ayant aucune date exploitable
        df = df.dropna(subset=['Dates_Propres'])

    # SÉCURISATION : Utilisation prioritaire des colonnes pré-calculées de votre fichier Excel
    ...
    # Traduction systématique en Français (Indépendant de la langue du serveur Streamlit)
...
# ========================================================
# ENTÊTE : SYSTEME DE FILTRES TEMPORELS GLOBAUX (EN HAUT)
# ========================================================
...
# Application globale du filtre temporel corrigé
